...
For production, deploy backend + frontend together on one public domain and use same-origin API calls (`/api/...`).
...
- `FRONTEND_URL` = `https://eudyaan-web.onrender.com`
- `ALLOWED_ORIGINS` = `https://eudyaan-web.onrender.com`
- `ADMIN_EMAIL` = your admin email
- `ADMIN_PASSWORD_HASH` = bcrypt hash (generated by script)
- `ANON_ID_SECRET` = long random secret

Rules:

- Use full `https://` URLs.
- Do not add trailing `/`.
- Do not wrap env values in quotes.

After updating env vars, redeploy/restart the service.
...
- AI Assistant shows `Temporary issue. Please try again.` repeatedly:

	This usually means login/session token is missing or expired for secured AI endpoints.

	Fix:
	1. Login again.
	2. Hard refresh (`Ctrl+F5`).
	3. Confirm Render env vars above are set correctly.
