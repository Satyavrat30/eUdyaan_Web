...
const chatSeriousFilter = document.getElementById("chatSeriousFilter");
...
const CHAT_SERIOUS_FILTER_KEY = "eudyaan_admin_chat_serious_filter";
...
function renderChats(items) {
  if (!items.length) {
    chatsBody.innerHTML = `<tr><td colspan="6">No chats found for selected filter.</td></tr>`;
    return;
  }
...
async function loadDashboard() {
  ...
  try {
    ...
    const selectedChatSerious = chatSeriousFilter?.value || "all";
    const [summaryData, chatsData, riskData, communityData, contactsData, appointmentsData] = await Promise.all([
      ...
      apiGet(`/api/admin/dashboard/chats?limit=200&serious=${encodeURIComponent(selectedChatSerious)}`),
...
const savedChatSeriousFilter = localStorage.getItem(CHAT_SERIOUS_FILTER_KEY);
if (savedChatSeriousFilter === "all" || savedChatSeriousFilter === "true") {
  chatSeriousFilter.value = savedChatSeriousFilter;
}
...
chatSeriousFilter?.addEventListener("change", () => {
  localStorage.setItem(CHAT_SERIOUS_FILTER_KEY, chatSeriousFilter.value);
  loadDashboard();
});
