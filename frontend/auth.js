...
// Fix #4 + #11: anonymousId is now derived server-side via HMAC.
// On the frontend we just store whatever the server returns.
function saveLoggedInProfile(user, sessionToken) {
  ...
  // Fix #2: persist the server-issued session token separately
...
if (signupBtn) {
  ...
  signupBtn.addEventListener("click", async () => {
    ...
    if (!name || !email || !password) { setMessage("Please fill name, email, and password.", true); return; }
    if (confirmPassword && password !== confirmPassword) { setMessage("Passwords do not match.", true); return; }
    ...
    try {
      ...
      if (result.success) {
        if (result.emailSent) {
          ...
          // Fix #15: show resend link after signup
          const msgEl = document.getElementById("authMessage");
          if (msgEl) {
            msgEl.innerHTML += ` <a href="#" id="resendVerifyLink" style="color:#1d4ed8;">Resend verification email</a>`;
            document.getElementById("resendVerifyLink")?.addEventListener("click", async (e) => {
              e.preventDefault();
              try {
                await postJson(`${API_BASE}/api/auth/resend-verification`, { email });
                setMessage("A new verification email has been sent.");
              } catch (err) {
                setMessage(err.message || "Failed to resend.", true);
              }
            });
          }
        } else {
...
if (loginBtn) {
  loginBtn.addEventListener("click", async () => {
    ...
    if (!email || !password) { setMessage("Please enter email and password.", true); return; }

    try {
      const result = await postJson(`${API_BASE}/api/auth/login`, { email, password });
      if (result.success) {
        // Fix #2: save the server-issued sessionToken
        saveLoggedInProfile(result.user || { email, name: email.split("@")[0] }, result.sessionToken);
...
// FORGOT PASSWORD — Fix #15: includes resend verification option
...
// RESEND VERIFICATION (standalone button, e.g. on a dedicated page)
...
if (resendVerifyBtn) {
  resendVerifyBtn.addEventListener("click", async () => {
    ...
    if (!email) { setMessage("Please enter your email.", true); return; }
    try {
      const result = await postJson(`${API_BASE}/api/auth/resend-verification`, { email });
      setMessage(result.message || "Sent.", false);
    } catch (error) {
      setMessage(error.message || "Request failed.", true);
    }
  });
...
// RESET PASSWORD — Fix #14: UI tells user a confirmation email was sent
...
if (resetPasswordBtn) {
  ...
  resetPasswordBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (result.success) {
        setMessage(result.message + " A confirmation email has been sent. Redirecting to login...");
...
if (window.location.pathname.includes("login.html")) {
  const params = new URLSearchParams(window.location.search);
  if (params.get("verified") === "1") setMessage("Email verified! You can now login.");
}
