...
const TEST_CLIENT_IP = process.env.TEST_CLIENT_IP || `10.77.${Math.floor(Math.random() * 200) + 20}.${Math.floor(Math.random() * 200) + 20}`;
...
async function run() {
  ...
  userToken = getRequired(login.data?.sessionToken);
  state.createdUserId = getRequired(login.data?.user?.id);
  ok(
    login.status === 200 && login.data?.success === true && /^[a-fA-F0-9]{24}$/.test(state.createdUserId) && !!userToken,
    "User login returns valid id",
    state.createdUserId
  );
  state.createdAnonymousId = "";

  if (!userToken) {
    console.log("\n===== HARDCORE API TEST SUMMARY =====");
    console.log(`Passed: ${state.pass}`);
    console.log(`Failed: ${state.fail}`);
    console.log(`Skipped: ${state.skip}`);
    process.exitCode = 1;
    return;
  }
  ...
  const appointmentList = await request("/api/appointments", {
  ...
  const communityPostsBefore = await request("/api/community/posts?sort=recent&category=all&days=all&tag=all", {
    headers: userAuthHeaders
  });
  ...
  state.createdAnonymousId = getRequired(createPost.data?.post?.anonymousId, state.createdAnonymousId || makeAnonId(state.createdUserId));
  ...
  ok([200, 429, 502].includes(aiCall.status), "AI support runtime call handled", `status=${aiCall.status}`);
  ...
  let adminToken = "";
  if (!ADMIN_PASSWORD) {
    skip("Admin login succeeds", "Set ADMIN_TEST_PASSWORD in env to run admin-authenticated checks");
    skip("Admin /me accepts valid token");
    skip("Admin anonymous lookup works for created user");
    skip("Admin login rejects invalid password");
    skip("Admin chats endpoint works");
    skip("Admin community endpoint works");
    skip("Admin contacts endpoint works");
    skip("Admin appointments endpoint works");
    skip("Dashboard summary endpoint works");
    skip("Summary includes split risk-alert counters");
    skip("Community risk-alert filter endpoint works");
    skip("Community filter contains newly logged alert");
    skip("AI risk-alert filter endpoint works");
    skip("AI filter returns only AI sourceType alerts");
    skip("Risk-alert pagination endpoints work");
    skip("Risk-alert pagination respects limit");
    skip("Admin login brute-force protection returns 429");
    skip("Admin logout works");
  } else {
    ...
    if (!adminToken) {
      skip("Admin /me accepts valid token", "Skipped because admin login did not return token");
      skip("Admin anonymous lookup works for created user");
      skip("Admin chats endpoint works");
      skip("Admin community endpoint works");
      skip("Admin contacts endpoint works");
